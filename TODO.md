...
- [x] Update `send_welcome_email()` function with new message:
...
- [x] Create new `send_order_confirmation_email()` function:
...
- [x] Import `send_order_confirmation_email` in views
- [x] Call `send_order_confirmation_email()` in `process_payment()` function after order is confirmed

### 3. Testing
- [x] Test user signup email
- [x] Test distributor signup email
